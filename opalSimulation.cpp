...
namespace opal {
	OpalSimulation::OpalSimulation(OpalSceneManager* m) {
		...
		this->resultsMessage="OpalSimulation";
	...
	void OpalSimulation::checkLaunchSize(unsigned int w, unsigned int h, unsigned int d) {
		...
		if ((w*h*d) >= 1073741824) {
			LOG_S(ERROR)<< "launch dimensions cannot exceed 2^30. Launched with w="<<w<<"h="<<h<<"d="<<d<<std::endl;
	...
	void OpalSimulation::finishSceneContext() {
		...
		myManager->getContext()->setMissProgram(reflectionRayIndex, myManager->getDefaultProgram("miss"));
		...
		//Miss programs require ray_type_index instead of entry_point_index
			myManager->getContext()->setMissProgram(traceRayIndex, myManager->getDefaultProgram("missLogTrace"));
	...
	void OpalSimulation::processHits(HitInfo* host_hits, uint hits) {
		...
		if (hits==0) {
			std::cout<<"\t"<<resultsMessage<<":  hits="<<hits<<std::endl;	
			return;
		}
		
		std::cout<<"\t"<<resultsMessage<<" Reflection hits="<<hits<<std::endl;	
		...
		for (uint i=0; i<hits; i++) {
			...
			if (host_hits->thrd.x!=currentTx) {
				if (raysHit!=0) {
					...
					if (info) {	
						info->updateField(E, Ex,Ey,Ez,receivers[index]->externalId,activeTransmitters[currentTx]->externalId,index,raysHit, 0); 						}	
			...
			for (auto cb : callbacks) {
				HitRecord hr=toHitRecord(host_hits);
				//std::cout<<"opalSimulation:: hr="<<hr.toString()<<std::endl;
				cb(activeTransmitters[currentTx]->externalId,receivers[index]->externalId, hr);
				//cb(activeTransmitters[currentTx]->externalId,receivers[index]->externalId, host_hits);
			}
			++raysHit;
			++host_hits;
	...
	HitRecord OpalSimulation::toHitRecord(HitInfo* host_hits) {
		HitRecord hr;
		hr.E=make_float2(host_hits->EEx.x,host_hits->EEx.y);
		hr.Ex=make_float2(host_hits->EEx.z,host_hits->EEx.w);
		hr.Ey=make_float2(host_hits->EyEz.x,host_hits->EyEz.y);
		hr.Ez=make_float2(host_hits->EyEz.z,host_hits->EyEz.w);
		float3 directionOfArrival=make_float3(host_hits->doaD.x,host_hits->doaD.y,host_hits->doaD.z);
		float3 directionOfDeparture=make_float3(host_hits->rdud.x,host_hits->rdud.y,host_hits->rdud.z);
		hr.directionOfDeparture=directionOfDeparture;
		hr.directionOfArrival=directionOfArrival;
		hr.unfoldedPath=host_hits->doaD.w;
		hr.diffraction=false;
		return hr;
	}
	void  OpalSimulation::printHitInfo(HitInfo* host_hits, uint hits) {
		...
		for (uint i=0; i<hits; i++) {
			...
			std::cout<<i<<"\tE="<<make_float2(host_hits->EEx.x,host_hits->EEx.y)<<std::endl;
	...
	void OpalSimulation::setCallback(std::function<void(int, int, HitRecord)> callback) {

		this->callbacks.push_back(callback);
	}
	void OpalSimulation::clearCallbacks() {
