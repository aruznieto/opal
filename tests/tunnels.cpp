...
void CubeTunnelTests::cubeTunnelRDNIsotropic(float distance) {
	...
	RayDensityNormalizationSimulation* sim=new RayDensityNormalizationSimulation(sceneManager);// Default uses NOMEN
	//Alternatively, other RDN methods can be used
	//RayDensityNormalizationSimulation* sim=new RayDensityNormalizationSimulation(sceneManager, RDNExecutionMode::HITINFO);
	//Default RDN method is much faster
	
	
	//Deprecated. It is ignored. Now, it records both the induced voltage in the antenna and the (projections on the axis of) incident field
	...
	//Alternative ways of setting the ray sphere 
	...
	//We only generate rays in a half hemisphere forward (azimuth -90 to 90), since we only want to transmit inside the tunnel and the transmitter is at the entrace
	int rayD=10000;
...
void CubeTunnelTests::cubeTunnelRDN(float distance) {
	...
	//Alternative generation method. Less efficient
	//OpalRaySphereGenerator* gen=sceneManager->getRaySphereGenerator();
	...
	int launches=1;
	//This technique allows to launch with extremely high ray density, it launches by solid angle sectors with very high density per solid angle
...
//	sceneManager->getSimulation()->setPrintHits(true);	
	...
	//Receiver polarization
	optix::float3 polarization = V; 
	//At 100 m it works well
	float distance=100.0f;
	// But at 900 m this technique is not accurate, does not work well. RDN (see above), on the contrary, works well at large distances
	//float distance=900.0f;
