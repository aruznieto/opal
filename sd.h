...
namespace opal {
	class  SingleDiffraction: public  OpalSimulation {
		protected:
			...
			virtual optix::Buffer setHitBuffer(uint ne, uint nrx,uint ntx);
			...
			virtual void processDiffractionLaunch(std::vector<SphereReceiver*>& rx);
			...
			void setReducedBuffersSize(std::vector<SphereReceiver*> rx, uint ntx, uint ne); 
			void separateLaunches(uint ntx, uint ne);
