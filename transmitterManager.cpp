...
void TransmitterManager::addTransmitterToGroup(int txId, float transmitPower, optix::float3 origin, optix::float3 polarization, float frequency) {
	ConfigurationOptions options = myManager->getConfigurationOptions();
	if (options.useMultiChannel ==false) {
			throw  opal::Exception("TransmitterManager::addTransmitterToGroup():  multichannel not enabled, enable it to pass frequency as parameter");
	}
	//Find transmitter 
	Transmitter* tb= getTransmitter(txId);
	tb->origin_p=make_float4(origin,transmitPower);
	float k= 2* 3.14159265358979323846f *frequency/myManager->getChannelParameters().c;
	tb->polarization_k =make_float4(polarization, k);
//	tb->txPower=transmitPower;
