...
typedef struct {float4 EEx; float4 EyEz; 
	/*Compute additional output. Remove if performance requires it*/ 
	float4 doaD; //Packed [(float3) direction of arrival, unfolded path distance] 
	float4 doDu; //Packed [(float3) direction of departure, distance from diffraction point to rx] distance from tx to diffraction point can be computed as s'=doaD.w - doDu.w
} RDNHit;
