...
//RDN with power trace using NOMEM. We accumulate here with atomicAdds the field for each receiver on the eBuffer
RT_PROGRAM void closestHitReceiverCurved()
{
	...
	float unfoldedPathLength = ds.x + hitPayload.lrhpd.w;
	...
	int addJump=0;
	if (reflections==0) {
		//unfoldedPathLength = length(current_tx.origin-prx) + hitPayload.lrhpd.w;	
	...
	//The coherent coefficient (X_P^c in Didascalou notation) is modified below to take into account that our M is related to Didascalou M' as  M=M'*2. Then sqrt(initialDensity*rayDensity/(M'
	...
	if (computeMode==1) {
		...
		atomicAdd(&eBuffer[index].EyEz.x,Eipar.x);
		atomicAdd(&eBuffer[index].EyEz.y,Eipar.y);
		atomicAdd(&eBuffer[index].EyEz.z,Eirad.x);
		atomicAdd(&eBuffer[index].EyEz.w,Eirad.y);
	...
	if (computeMode==0) {
		...
		atomicAdd(&eBuffer[index].EEx.x,E.x);
		atomicAdd(&eBuffer[index].EEx.y,E.y);
	}
