...
*/
	...
	class GeometryUtils {
		public: 
			//Usually used to change from world to local. Homogeneous matrix. Pass orthogonal vectors in world coordinates
			static optix::Matrix4x4 changeOfBasis(optix::float3 x, optix::float3 y, optix::float3 z);
			static optix::Matrix4x4 transformToBasis(optix::float3 x, optix::float3 y, optix::float3 z, optix::float3 p);
			static optix::float3 transformVector(optix::float3 v, optix::float3 x, optix::float3 y, optix::float3 z);
			static optix::float3 transformPoint(optix::float3 p, optix::float3 x, optix::float3 y, optix::float3 z);
			static optix::float3 transformPoint(optix::float3 p, optix::Matrix4x4&  tm);
			static optix::float3 transformVector(optix::float3 v, optix::Matrix4x4&  tm);
			static float orientedAngle2D(optix::float2 orientation, optix::float2  v);
			static optix::float2 getAngles(optix::float3 ray);
			static optix::float3 projectPointOnPlane(optix::float3 normal, optix::float3 point);
