/***************************************************************/
//
//Copyright (c) 2020 Esteban Egea-Lopez http://girtel.upct.es/~simulations
//
/**************************************************************/
...
namespace opal {
	...
	void RayDensityNormalizationSimulation::setCallback(std::function<void(int, int, HitRecord)> callback) {
		if (execMode==HITINFO) {
			OpalSimulation::setCallback(callback);
		} else {
			throw  opal::Exception("hit info callback with RayDensityNormalizationSimulation can only be used with execMode=HITINFO");
		}
	}	
	...
	void RayDensityNormalizationSimulation::executeReceivedFieldLaunch(unsigned int numTransmitters) {
		...
		myManager->getContext()->launch(reflectionEntryIndex, raySphere.elevationSteps, raySphere.azimuthSteps,numTransmitters); //Launch 3D (elevation, azimuth, transmitters);
		time.stop();
		std::cout<<"Launch finished. Time="<<time.getTime()<<std::endl;
	...
	void RayDensityNormalizationSimulation::processReceivedFieldBuffer() {
		...
		for (unsigned int x=0; x<receivers.size(); ++x) {
			for (unsigned int y=0; y<activeTransmitters.size(); ++y) {
				...
				//std::cout<<"RayDensityNormalizationSimulation::processReceivedFieldBuffer() rxId="<<receivers[x]->externalId<<"; E="<<E<<"Ex="<<Ex<<"Ey="<<Ey<<"Ez="<<Ez<<std::endl;
				myManager->getFieldInfo()->updateField(E, Ex,Ey,Ez,receivers[x]->externalId,activeTransmitters[y]->externalId,x,-1,0); 			
			}
		}
	}
	void RayDensityNormalizationSimulation::processFixedRayBuffer() {
		...
		for (unsigned int x = 0; x < w; ++x)
		{
			...
			myManager->getFieldInfo()->updateField(E, Ex,Ey,Ez,receivers[x]->externalId,activeTransmitters[0]->externalId,x,-1,0); 			
		}
	...
	void RayDensityNormalizationSimulation::processHits(HitInfo* h, uint hits) {
		...
		for (uint i=0; i<hits; i++) {
			...
			myManager->getFieldInfo()->updateField(E,Ex,Ey,Ez,receivers[index]->externalId,activeTransmitters[currentTx]->externalId,index,1,0); 			
			if (generateTraceLog) {
