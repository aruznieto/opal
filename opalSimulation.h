...
namespace opal {
	...
	class OpalSimulation {
		protected:
			...
			//Callbacks to work directly with the hits recorded
		...
		public:
			...
			//Function to be called, if necessary, to get additional hit info 
		        virtual void setCallback(std::function<void(int, int, HitRecord)> callback);	
			virtual HitRecord toHitRecord(HitInfo* host_hits);
			//Launch functions
			//Make abstract 		
